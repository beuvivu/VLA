...
def test_same_draw_pair_is_labeled_as_statistical_cooccurrence() -> None:
    ...
    assert bool(out.loc[0, "cap_loto_50_related"]) is True
    assert out.loc[0, "cap_loto_50_kind"] == "reverse"
    assert bool(out.loc[1, "reverse_related"]) is False
    assert bool(out.loc[1, "cap_loto_50_related"]) is False
...
def test_reversal_artifact_relation_metadata() -> None:
    ...
    assert bool(row["cap_loto_50_related"]) is True
    assert row["cap_loto_50_id"] == "38-83"
    assert row["cap_loto_50_kind"] == "reverse"
    assert bool(row["same_bo_family"]) is True
    assert "reverse" in row["relation_tags"]
    assert "cap_loto_50" in row["relation_tags"]


def test_kep_bong_pair_metadata() -> None:
    raw = pd.DataFrame([{"a": 0, "b": 55, "count": 4}])
    out = annotate_pair_frame(
        raw, a_col="a", b_col="b", pair_kind="same_draw_cooccurrence"
    )
    row = out.iloc[0]
    assert row["canonical_pair_key"] == "00-55"
    assert bool(row["reverse_related"]) is False
    assert bool(row["bong_duong_related"]) is True
    assert bool(row["cap_loto_50_related"]) is True
    assert row["cap_loto_50_id"] == "00-55"
    assert row["cap_loto_50_kind"] == "kep_bong"
    assert "kep_bong" in row["relation_tags"]


def test_bong_am_relation_metadata_is_explicit_but_not_a_50_pair() -> None:
    raw = pd.DataFrame([{"a": 14, "b": 41, "count": 2}])
    out = annotate_pair_frame(
        raw, a_col="a", b_col="b", pair_kind="same_draw_cooccurrence"
    )
    row = out.iloc[0]
    assert bool(row["reverse_related"]) is True
    assert bool(row["bong_am_related"]) is True
    # 14-41 is also a reverse pair, therefore it is in the 50-pair partition.
    assert bool(row["cap_loto_50_related"]) is True
    assert "bong_am" in row["relation_tags"]
