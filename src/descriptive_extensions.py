...
def build_head_table(two: pd.DataFrame, *, lookback_days: int) -> pd.DataFrame:
    view = two.tail(max(1, int(lookback_days)))
...
def _group_gap_rows(two: pd.DataFrame, *, mode: str) -> tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame]:
    ...
    def gap_for(predicate) -> tuple[int, str | None, int]:
        idx = [i for i, nums in enumerate(day_sets) if any(predicate(n) for n in nums)]
        if not idx:
            return len(day_sets), None, 0
        last_i = idx[-1]
        return int((latest - dates.iloc[last_i]).days), dates.iloc[last_i].date().isoformat(), len(idx)

    sum_rows: list[dict[str, object]] = []
    for total in range(19):
        gap, last_date, hits = gap_for(lambda n, total=total: digit_sum(n) == total)
        sum_rows.append(
            {
                "mode": mode,
                "digit_sum": total,
    ...
    for total in range(10):
        gap, last_date, hits = gap_for(lambda n, total=total: digit_sum_mod10(n) == total)
        mod_rows.append(
            {
                "mode": mode,
                "sum_mod10": total,
    ...
    for d in range(10):
        gap, last_date, hits = gap_for(lambda n, d=d: d in (n // 10, n % 10))
        touch_rows.append(
            {
                "mode": mode,
                "digit": d,
    ...
    return (
        pd.DataFrame(sum_rows).sort_values("gap_days", ascending=False, ignore_index=True),
        pd.DataFrame(mod_rows).sort_values("gap_days", ascending=False, ignore_index=True),
        pd.DataFrame(touch_rows).sort_values("gap_days", ascending=False, ignore_index=True),
    )


def build_number_recency(two: pd.DataFrame, *, mode: str, recent_dates: int = 8) -> pd.DataFrame:
    dates = _date_series(two)
    latest = dates.iloc[-1]
    if mode == "de":
        day_sets = [{int(x)} for x in _de_values(two)]
    ...
    for n, idx in enumerate(appearances):
        if idx:
            ...
            last_i = idx[-1]
            recent = [dates.iloc[i].date().isoformat() for i in idx[-recent_dates:][::-1]]
    ...
    return pd.DataFrame(rows).sort_values(["days_since", "hit_days"], ascending=[False, False], ignore_index=True)


def build_pair_recency(two: pd.DataFrame, *, top: int = 300, recent_dates: int = 6) -> pd.DataFrame:
    dates = _date_series(two)
        ...
        rows.append(
            {
                ...
                "lift": lift,
                "last_date": dates.iloc[last_i].date().isoformat(),
                "days_since": int((dates.iloc[-1] - dates.iloc[last_i]).days),
                "recent_dates": json.dumps(
                    [dates.iloc[j].date().isoformat() for j in idx[-recent_dates:][::-1]],
                    ensure_ascii=False,
                ),
            }
        )
    ...
    return (
        pd.DataFrame(rows)
        .sort_values(["lift", "cooccurrence_days"], ascending=[False, False])
        .head(top)
...
def main() -> None:
    ap = argparse.ArgumentParser(description="Extended descriptive/recency statistics.")
    ...
    windows = sorted({max(1, int(x)) for x in args.head_windows.split(",") if x.strip()})
    for window in windows:
        build_head_table(two, lookback_days=window).to_csv(out / f"head_table_{window}d.csv", index=False)
    ...
    manifest = {
        ...
        "note": "Descriptive evidence only; these outputs do not alter production prediction weights.",
    }
