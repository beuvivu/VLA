...
Critical CSS is auto-generated by scripts/extract_critical_css.py
from docs/assets/ui.css (shell selectors + used design tokens only).
"""
...
_FALLBACK_CRITICAL = (
    ...
    ".ui-nav a{display:inline-flex;padding:.375rem .75rem;border-radius:.75rem;"
    "border:1px solid var(--ui-border);background:var(--ui-surface);color:var(--ui-ink-soft);"
    ...
    ".ui-card-head{padding:1rem 1.25rem;border-bottom:1px solid var(--ui-border)}"
    ".ui-card-body{padding:1.25rem}.ui-grid{display:grid;gap:1.25rem}"
    ".ui-table-wrap{overflow:auto;max-width:100%}"
    ".ui-badge{display:inline-flex;padding:.15rem .55rem;border-radius:999px;font-size:.75rem;font-weight:600}"
    ".ui-badge-brand{background:var(--ui-brand-soft);color:var(--ui-brand-ink)}"
    ".ui-dock,.dock{position:fixed;left:50%;bottom:16px;transform:translateX(-50%);z-index:60;pointer-events:none}"
    "@media (max-width:640px){.ui-shell,.main{padding-left:14px;padding-right:14px}"
...
def stylesheet_link() -> str:
    """CRP-friendly head links: auto critical inline + async full CSS + font preload."""
    parts = [
