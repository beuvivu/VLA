...
Builders remain responsible for data and semantic markup.  This output-layer
compositor only adds visual hierarchy, responsive wrappers and dock clearance.
It never changes statistical values, table ids or JavaScript data hooks.
"""
...
_STYLE_RE = re.compile(
    rf'<style id="{re.escape(_STYLE_ID)}">.*?</style>',
    re.I | re.S,
)
...
.sp-page-bang-dac-biet #sp-grid th,.sp-page-bang-dac-biet #sp-grid td{text-align:center;padding:.72rem .55rem}
.sp-page-bang-dac-biet #sp-grid th:first-child,.sp-page-bang-dac-biet #sp-grid td:first-child{text-align:left}

.sp-page-lo-gan .sp-scroll:has(#sp-grid),.sp-page-lo-gan .sp-scroll:has(#sp-pair-gan){width:100%;max-width:100%;max-height:34rem;overflow:auto}
...
_PATH_CSS = r"""
.path-page{min-height:100vh;padding-bottom:calc(var(--ui-dock-h,64px) + env(safe-area-inset-bottom) + 2rem);background:radial-gradient(circle at 8% 0%,rgba(59,130,246,.12),transparent 26rem),radial-gradient(circle at 92% 6%,rgba(139,92,246,.10),transparent 28rem),var(--bg)}
...
.path-page .path-table-scroll table{margin:0}.path-page .path-table-scroll th{top:0;z-index:3;box-shadow:0 1px 0 var(--line)}
...
@media(max-width:640px){.path-page .path-shell{padding:12px 12px calc(var(--ui-dock-h,64px) + env(safe-area-inset-bottom) + 2.5rem)}.path-page .path-hero{padding:.9rem;border-radius:15px}.path-page .card{border-radius:15px;padding:12px}.path-page .quick-grid{grid-template-columns:1fr 1fr}}
"""

_DASHBOARD_CSS = r"""
.ai-command-center{background:radial-gradient(circle at 12% -8%,color-mix(in srgb,var(--ui-brand) 16%,transparent),transparent 30rem),radial-gradient(circle at 90% 4%,color-mix(in srgb,#8b5cf6 12%,transparent),transparent 28rem),var(--ui-bg)}
...
.ai-status-strip{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:.8rem;margin:1rem 0 1.25rem}.ai-status-item{min-width:0;padding:.85rem 1rem;border:1px solid var(--ui-border);border-radius:var(--ui-r-lg);background:color-mix(in srgb,var(--ui-surface) 88%,transparent);box-shadow:var(--ui-sh-sm)}
.ai-status-item span{display:block;font-size:.66rem;font-weight:800;letter-spacing:.08em;text-transform:uppercase;color:var(--ui-ink-soft)}.ai-status-item strong{display:block;margin-top:.25rem;font-size:.92rem;color:var(--ui-ink)}
.ai-signal-grid{align-items:start}.ai-signal-grid>.ui-card:nth-child(-n+2){position:relative;overflow:hidden;border-color:var(--ui-brand-border);box-shadow:var(--ui-sh-md)}.ai-signal-grid>.ui-card:nth-child(-n+2)::before{content:"";position:absolute;inset:0 auto 0 0;width:3px;background:var(--ui-brand)}
.ai-signal-grid>.ui-card .ui-card-head h2{letter-spacing:-.015em}.ai-command-center .ui-table{font-variant-numeric:tabular-nums}
@media(max-width:760px){.ai-status-strip{grid-template-columns:1fr}.ai-command-center .ui-header::after{font-size:4.5rem}}
"""

_RESEARCH_CSS = r"""
.research-workspace{background:linear-gradient(180deg,color-mix(in srgb,#020617 7%,var(--ui-bg)) 0,var(--ui-bg) 30rem)}
.research-workspace .rl-hero{position:relative;overflow:hidden;padding:clamp(1.4rem,3vw,2.35rem);background:radial-gradient(circle at 88% 20%,rgba(56,189,248,.18),transparent 17rem),linear-gradient(135deg,#07111f,#0f172a 58%,#111827);border:1px solid #263449;box-shadow:0 24px 65px rgba(2,6,23,.28)}
...
.rl-pipeline{display:grid;grid-template-columns:repeat(5,minmax(0,1fr));gap:.65rem;margin:0 0 1.25rem}.rl-stage{position:relative;padding:.85rem .8rem;border:1px solid var(--ui-border);border-radius:var(--ui-r-lg);background:var(--ui-surface);box-shadow:var(--ui-sh-sm)}
...
.rl-instruments>.ui-card{position:relative;overflow:hidden}.rl-instruments>.ui-card::before{content:"";position:absolute;left:0;top:0;bottom:0;width:2px;background:color-mix(in srgb,var(--ui-brand) 55%,transparent)}
.research-workspace .metric-card{position:relative;overflow:hidden;background:linear-gradient(180deg,var(--ui-surface),var(--ui-surface-2))}.research-workspace .metric-card::after{content:"";position:absolute;right:.85rem;top:.85rem;width:.46rem;height:.46rem;border-radius:50%;background:#22c55e;box-shadow:0 0 0 4px rgba(34,197,94,.1)}
@media(max-width:900px){.rl-pipeline{grid-template-columns:1fr 1fr}.rl-stage::after{display:none}}@media(max-width:560px){.rl-pipeline{grid-template-columns:1fr}}
"""

_LIVE_CSS = r"""
.live-console{padding-bottom:calc(var(--ui-dock-h,64px) + env(safe-area-inset-bottom) + 1rem)}.live-console .wrap{max-width:1120px;padding:24px 18px 36px}.live-console .wrap>.top{padding:1rem 1.1rem;border:1px solid rgba(148,163,184,.16);border-radius:18px;background:linear-gradient(135deg,rgba(13,27,45,.92),rgba(18,34,54,.72));box-shadow:0 18px 50px rgba(0,0,0,.18)}
.live-console .card{border-color:rgba(148,163,184,.18);background:linear-gradient(180deg,rgba(13,27,45,.96),rgba(13,27,45,.84));box-shadow:0 16px 44px rgba(0,0,0,.16)}.live-console .prize{min-height:3.4rem}.live-console .ui-live-nav{margin-top:1rem;padding-bottom:calc(var(--ui-dock-h,64px) + env(safe-area-inset-bottom) + 2.5rem)}
...
def _refine_live(page: str) -> str:
    return _append_style(_add_body_classes(page, "live-console"), _LIVE_CSS)
