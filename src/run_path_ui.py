...
from path_prob import fit_paths, predict_from_fitted_paths, predict_from_fitted_paths_full, paths_to_dataframe
...
def _build_path_lines(
    *,
    ...
    # Predict next-day number for each path
    next_date = anchor_date + pd.Timedelta(days=1)
    # Robust: next_date may be pandas Timestamp, datetime, or date
    try:
        # pandas Timestamp
        next_date = _to_date(next_date)
    except AttributeError:
        import datetime as _dt
        if isinstance(next_date, _dt.datetime):
            next_date = next_date.date()
        elif isinstance(next_date, _dt.date):
            next_date = next_date
        else:
            next_date = _dt.date.fromisoformat(str(next_date))

    pred_nums = []
    for _, r in stats_df.iterrows():
        lag = int(r["lag"])
        base_date = next_date - pd.Timedelta(days=lag)
...
def _save_picks(out_dir: Path, mode: str, pred_active: pd.DataFrame, pred_stable: pd.DataFrame) -> None:
    # Bach thu (top 1), song thu (top 2), dan top 10
    ...
    (out_dir / f"picks_{mode}.json").write_text(pd.Series(out).to_json(force_ascii=False, indent=2), encoding="utf-8")


def main() -> None:
    ap = argparse.ArgumentParser(description="Website-style PATH report (rawdata -> numbers).")

    ap.add_argument("--mode", choices=["loto", "de"], default="loto")

    # Website-like controls
    ap.add_argument("--bien-ngay", type=str, default=None, help="Anchor date (YYYY-MM-DD). Default: last date in data")
    ap.add_argument("--so-ngay", type=int, default=10, help="Display days (like 'Số ngày cầu chạy' in UI)")
    ap.add_argument("--scope", choices=["all", "near_special", "special_only"], default="all")
    ...
    # Bias toward special digits ("gần ĐB")
    ...
    # Clamp so-ngay like many UI (1..20) but allow larger if user wants
    ...
    dates = [_to_date(d) for d in pd.to_datetime(df_raw["date"])]

    anchor = _parse_date(args.bien_ngay) if args.bien_ngay else None
    anchor_date = _resolve_anchor_date(dates, anchor)

    # Fit paths once, then build both active/stable tables
    stats, raw_by_date, _dates = fit_paths(
        df_raw=df_raw,
        df_2digits=df_2d,
        params=params,
        mode=args.mode,
    ...
    # Filter stable / active
    stable_df = stats_df[(stats_df["trials"] >= params.min_trials) & (stats_df["max_streak"] >= params.min_max_streak)].copy()
    active_df = stats_df[(stats_df["trials"] >= params.min_trials) & (stats_df["current_streak"] >= params.min_current_streak)].copy()
    ...
    # UI-like line tables (last N days numbers + hit flags)
    ...
    stable_lines.to_csv(out_dir / f"ui_{args.mode}_stable_{anchor_date.isoformat()}_{display_days}d.csv", index=False)
    active_lines.to_csv(out_dir / f"ui_{args.mode}_active_{anchor_date.isoformat()}_{display_days}d.csv", index=False)

    # Next-day predicted numbers (aggregated)
    pred_active = predict_from_fitted_paths(
        stats=stats, raw_by_date=raw_by_date, dates=_dates, params=params, kind="active", mode=args.mode,
        top_numbers=args.top_numbers, anchor_date=anchor_date, scope=args.scope,
        bias_special_touch=args.bias_special_touch, bias_special_both=args.bias_special_both,
    )
    pred_stable = predict_from_fitted_paths(
        stats=stats, raw_by_date=raw_by_date, dates=_dates, params=params, kind="stable", mode=args.mode,
        top_numbers=args.top_numbers, anchor_date=anchor_date, scope=args.scope,
        bias_special_touch=args.bias_special_touch, bias_special_both=args.bias_special_both,
    )

    pred_active.to_csv(out_dir / f"predict_next_{args.mode}_active_{anchor_date.isoformat()}.csv", index=False)
    pred_stable.to_csv(out_dir / f"predict_next_{args.mode}_stable_{anchor_date.isoformat()}.csv", index=False)

    
    # Also compute & save full 00–99 probability tables (needed for ensemble learning)
    try:
        pred_active_full = predict_from_fitted_paths_full(
            stats=stats, raw_by_date=raw_by_date, dates=_dates, params=params, kind="active", mode=args.mode,
            anchor_date=anchor_date, scope=args.scope,
            bias_special_touch=args.bias_special_touch, bias_special_both=args.bias_special_both,
        )
        ...
        pred_active_full.to_csv(out_dir / f"predict_next_{args.mode}_active_{anchor_date.isoformat()}_all.csv", index=False)
        pred_stable_full.to_csv(out_dir / f"predict_next_{args.mode}_stable_{anchor_date.isoformat()}_all.csv", index=False)
    except Exception as _e:
        print(f"[WARN] could not export full predict tables: {_e}")
    ...
    (out_dir / f"path_manifest_{args.mode}.json").write_text(json.dumps(manifest, indent=2), encoding="utf-8")

    _save_picks(out_dir, args.mode, pred_active, pred_stable)

    print(f"Anchor date: {anchor_date} | mode={args.mode} | scope={args.scope} | display_days={display_days}")
