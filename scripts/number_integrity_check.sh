...
numbers = pd.read_csv("data/reference/number_ontology_00_99.csv", dtype={"number_str": str, "reverse": str, "bong_duong": str, "bo_family_id": str})
assert len(numbers) == 100
assert set(numbers["number_str"]) == {f"{n:02d}" for n in range(100)}
assert numbers["bo_family_id"].nunique() == 15

unique = pd.read_csv("data/reference/bo_unique_families.csv", dtype={"canonical_family_id": str})
assert len(unique) == 15
assert (unique["family_size"] == 4).sum() == 5
assert (unique["family_size"] == 8).sum() == 10

pair = pd.read_csv("data/descriptive_ext/pair_recency_loto.csv", dtype={"a_str": str, "b_str": str, "bo_family_id_a": str, "bo_family_id_b": str})
...
rev = pd.read_csv("data/pairs/reversal_pair_cooccurrence.csv", dtype={"a_str": str, "b_str": str})
